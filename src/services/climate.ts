...
export const WMO_CODES: Record<
  number,
  { label: string; emoji: string; severe: boolean }
> = {
...
export async function getWeatherForecast(params: {
  ...
  const currentInfo = WMO_CODES[currentCode] ?? {
    label: "Unknown",
    emoji: "🌡️",
    severe: false,
  };
  ...
  return {
    ...
    current: {
      ...
      humidity:
        hourIdx >= 0 ? (data.hourly.relativehumidity_2m?.[hourIdx] ?? 0) : 0,
      windspeed: Math.round(cw.windspeed ?? 0),
      apparentTemp:
        hourIdx >= 0
          ? Math.round(
              data.hourly.apparent_temperature?.[hourIdx] ??
                cw.temperature ??
                0,
            )
...
export function scoreActivityWeather(
  ...
  if (isSevere) {
    score -= 60;
    warnings.push(
      `${weatherEmoji} Severe weather: ${dayForecast.weatherLabel}`,
    );
  }
  ...
  if (precipitationSum > 20) {
    ...
    score -= 15;
    warnings.push(
      `🌦️ Moderate rain (${precipitationSum.toFixed(1)} mm) – carry an umbrella`,
    );
  }
  ...
  if (tempMax > 42) {
    ...
    score -= 15;
    warnings.push(
      `☀️ Very hot (${tempMax}°C) – plan outdoor activities for early morning`,
    );
  }
  if (tempMin < 0) {
    score -= 20;
  ...
  if (windspeedMax > 60) {
    score -= 25;
    warnings.push(
      `💨 Strong winds (${windspeedMax.toFixed(0)} km/h) – outdoor activities risky`,
    );
  ...
  if (score >= 80) recommendation = "Excellent conditions — go for it!";
    ...
    recommendation = "Acceptable — plan around weather windows.";
  else if (score >= 20)
    recommendation = "Poor conditions — consider rescheduling.";
...
export async function getActivityWeatherSuitability(params: {
  ...
  return activities.map((act) => {
    ...
    const { score, warnings, recommendation } = scoreActivityWeather(
      act.category,
      dayForecast,
    );
...
export async function getClimateAwareRoute(params: {
  ...
  try {
    ...
    const dayForecast =
      forecast.daily.find((d) => d.date === targetDate) ?? forecast.daily[0];

    if (dayForecast) {
      weatherAlongRoute = `${dayForecast.weatherEmoji} ${dayForecast.weatherLabel} · ${dayForecast.tempMin}–${dayForecast.tempMax}°C`;
      if (dayForecast.isSevere)
        avoidanceReasons.push("Severe weather at destination");
      if (dayForecast.precipitationSum > 15)
        avoidanceReasons.push("Heavy rain — allow extra travel time");
      if (dayForecast.windspeedMax > 50)
        avoidanceReasons.push("Strong winds may affect driving");
    }
  } catch {
    /* weather fetch failed – continue with route only */
  }

  // Get ORS route via server-side proxy
  const routeRes = await fetch("/api/openroute", {
    method: "POST",
  ...
  if (!routeRes.ok) {
    ...
    throw new Error(
      `ORS route error [${routeRes.status}]: ${errData?.error ?? "unknown"}`,
    );
  }
  ...
  switch (action) {
    case "weather-forecast":
      return getWeatherForecast(
        params as Parameters<typeof getWeatherForecast>[0],
      );

    case "activity-suitability":
      return getActivityWeatherSuitability(
        params as Parameters<typeof getActivityWeatherSuitability>[0],
      );

    case "climate-aware-route":
      return getClimateAwareRoute(
        params as Parameters<typeof getClimateAwareRoute>[0],
      );
    ...
    case "geocode":
      return geocodeDestination(
        (params as { destination: string }).destination,
      );
