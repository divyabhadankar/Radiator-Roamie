...
-- ============================================================
-- ██████████████████████████████████████████████████████████
--  ONE-STOP SCRIPT — THIS IS THE SINGLE SOURCE OF TRUTH
--  Contains EVERYTHING: all original tables from master_schema
--  (v2.0, Parts 0-9) PLUS the new user_live_locations table
--  for the "Nearby Travelers" live-location feature.
-- ██████████████████████████████████████████████████████████
-- HOW TO RUN
...
-- ============================================================
-- WHAT'S NEW IN v3.0 vs master_schema v2.0
--   • Table  2.16  user_live_locations  — latest GPS ping per user
--   • Indexes       idx_user_live_locations_user / _updated
--   • Function 4f   find_nearby_travelers()  — Haversine radius query
--   • RLS  7.16     four policies for user_live_locations
--   • Realtime      user_live_locations added to supabase_realtime
-- ============================================================
...
-- user_live_locations
DO $$ BEGIN
  IF to_regclass('public.user_live_locations') IS NOT NULL THEN
    DROP POLICY IF EXISTS "Authenticated users can view live locations" ON public.user_live_locations;
    DROP POLICY IF EXISTS "Users can upsert own live location"          ON public.user_live_locations;
...
-- ----------------------------------------------------------
-- 2.16 USER LIVE LOCATIONS
--      Stores the latest broadcasted location for each user
--      along with their active trip info (for nearby discovery).
--      Updated via upsert on each GPS ping.
-- ----------------------------------------------------------
...
-- user_live_locations indexes
CREATE INDEX IF NOT EXISTS idx_user_live_locations_user    ON public.user_live_locations(user_id);
...
-- 4f. Find users within a given radius (in km) using Haversine
CREATE OR REPLACE FUNCTION public.find_nearby_travelers(
  p_lat    DOUBLE PRECISION,
  p_lng    DOUBLE PRECISION,
  p_radius DOUBLE PRECISION DEFAULT 1.0,  -- km
...
AS $$
  SELECT
    ...
    (
      6371 * 2 * ASIN(
        SQRT(
          POWER(SIN((RADIANS(ull.lat) - RADIANS(p_lat)) / 2), 2) +
          COS(RADIANS(p_lat)) * COS(RADIANS(ull.lat)) *
          POWER(SIN((RADIANS(ull.lng) - RADIANS(p_lng)) / 2), 2)
        )
      )
    ) AS distance_km,
    ull.updated_at
  FROM public.user_live_locations ull
  WHERE
    (p_exclude_user_id IS NULL OR ull.user_id != p_exclude_user_id)
    AND ull.updated_at > now() - INTERVAL '5 minutes'
    AND (
      6371 * 2 * ASIN(
        SQRT(
          POWER(SIN((RADIANS(ull.lat) - RADIANS(p_lat)) / 2), 2) +
...
-- ----------------------------------------------------------
-- 7.16 user_live_locations
-- ----------------------------------------------------------

-- Any authenticated user can read live locations (needed for nearby discovery)
CREATE POLICY "Authenticated users can view live locations"
  ON public.user_live_locations FOR SELECT
  TO authenticated
  USING (true);

-- Users can only insert/update/delete their own location
CREATE POLICY "Users can upsert own live location"
...
BEGIN
  ...
  -- user_live_locations: realtime nearby traveler discovery
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables
    WHERE pubname = 'supabase_realtime'
    AND   schemaname = 'public'
...
-- Spot-check find_nearby_travelers (replace coords + uuid as needed):
-- SELECT * FROM public.find_nearby_travelers(28.6139, 77.2090, 5.0, NULL);
...
--
-- RLS policies (42 total across all tables):
--   38 original + 4 new for user_live_locations
--
-- Indexes (28 total):
--   26 original + idx_user_live_locations_user           ← NEW
--                + idx_user_live_locations_updated        ← NEW
--
-- Realtime tables (5):
--   messages, activity_votes, community_messages,
--   trip_join_requests,
--   user_live_locations                                  ← NEW
-- ============================================================
